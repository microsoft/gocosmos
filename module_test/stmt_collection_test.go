...
func TestStmtListCollections_Query(t *testing.T) {
	...
	db := _openDb(t, testName)
	dbname := "dbtemp"
	_, _ = db.Exec("DROP DATABASE IF EXISTS db_not_found")
	...
	for _, collName := range collNames {
		_, _ = db.Exec(fmt.Sprintf("CREATE COLLECTION %s.%s WITH pk=/id", dbname, collName))
	}
	...
	if !errors.Is(err, gocosmos.ErrNotFound) {
