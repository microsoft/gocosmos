...
func TestStmtInsert_Exec(t *testing.T) {
	...
	testData := []struct {
		...
		{
			name:         "insert_new_2",
			sql:          fmt.Sprintf(`INSERT INTO %s.tbltemp (id, username, email, grade, actived) VALUES ("\"2\"", "\"user2\"", "\"user2@domain1.com\"", 8, false) with SINGLE_PK`, dbname),
		...
		},
		{
			...
			sql:          fmt.Sprintf(`INSERT INTO %s.tbltemp (id,username,email,grade,actived) VALUES ("\"2\"", "\"user\"", "\"user@domain1.com\"", 9, false) WITH SINGLE_PK=true`, dbname),
			args:         []interface{}{"user"},
			mustConflict: true,
		},
		{
		...
		},
		{
			...
			sql:          fmt.Sprintf(`INSERT INTO %s.tbltemp (id, username, email, grade, actived, data) VALUES (:1, $2, @3, @4, $5, :6) WITH SINGLE_PK=true`, dbname),
		...
		},
		{
			...
			sql:          fmt.Sprintf(`INSERT INTO %s.tbltemp (id, username, email, grade, actived, data) VALUES (:1, $2, @3, @4, $5, :6) WITH singlePK=true`, dbname),
...
func TestStmtInsert_WithPK(t *testing.T) {
	...
	testData := []struct {
	...
	}{
		{
			...
			sql:          fmt.Sprintf(`INSERT INTO %s.tbltemp (id, username, email, grade, actived) VALUES ("\"1\"", "\"user\"", "\"user@domain1.com\"", 7, true) WITH pk=/username`, dbname),
			affectedRows: 1,
		},
		{
		...
		{
			name:         "insert_conflict_uk",
			sql:          fmt.Sprintf(`INSERT INTO %s.tbltemp (id,username,email,grade,actived) VALUES ("\"2\"", "\"user\"", "\"user@domain1.com\"", 9, false) WITH pk=/username`, dbname),
			mustConflict: true,
		},
		{
			name:         "table_not_exists",
		...
		{
			name: "insert_new_placeholders_explicit_pk",
			initSqls: []string{
				"DROP DATABASE IF EXISTS db_not_exists",
				fmt.Sprintf("DROP DATABASE IF EXISTS %s", dbname),
				fmt.Sprintf("CREATE DATABASE %s", dbname),
			...
			sql:          fmt.Sprintf(`INSERT INTO %s.tbltemp (id, username, email, grade, actived, data) VALUES (:1, $2, @3, @4, $5, :6) WITH pk=/username`, dbname),
		...
		{
			name:      "error_invalid_num_params",
			sql:       fmt.Sprintf(`INSERT INTO %s.tbltemp (id, username, email, grade, actived, data) VALUES (:1, $2, @3, @4, $5, :8) WITH pk=/username`, dbname),
...
func TestStmtInsert_WithPK_SubPartitions(t *testing.T) {
	...
	testData := []struct {
	...
	}{
		{
			...
			sql:          fmt.Sprintf(`INSERT INTO %s.tbltemp (id, app, username, email, grade, actived, data) VALUES (:1, $2, @3, :4, $5, @6, :7) WITH pk=/app,/username`, dbname),
